...
def translate_state_dict_key(k):
    ...
    if k == "model.embed_tokens.weight":
        ...
        if k.endswith(".self_attn.q_proj.weight"):
            return "layers.{}.attention.wq.weight".format(layer)
        elif k.endswith(".self_attn.k_proj.weight"):
            return "layers.{}.attention.wk.weight".format(layer)
        elif k.endswith(".self_attn.v_proj.weight"):
            return "layers.{}.attention.wv.weight".format(layer)
        elif k.endswith(".self_attn.o_proj.weight"):
            return "layers.{}.attention.wo.weight".format(layer)
        elif k.endswith(".mlp.gate_proj.weight"):
            return "layers.{}.feed_forward.w1.weight".format(layer)
        elif k.endswith(".mlp.down_proj.weight"):
            return "layers.{}.feed_forward.w2.weight".format(layer)
        elif k.endswith(".mlp.up_proj.weight"):
            return "layers.{}.feed_forward.w3.weight".format(layer)
        elif k.endswith(".input_layernorm.weight"):
            return "layers.{}.attention_norm.weight".format(layer)
        elif k.endswith(".post_attention_layernorm.weight"):
            return "layers.{}.ffn_norm.weight".format(layer)
...
def save_shards(model_sd, num_shards):
    ...
    with torch.no_grad():
        if num_shards == 1:
            ...
            print("Saving shard 1 of {} into {}/consolidated.00.pth".format(num_shards, output_dir))
            ...
            for k in list(model_sd.keys()):
                ...
                if new_k is not None:
                    if new_k=='tok_embeddings.weight':
                        ...
                        print("Unexpected key {}".format(new_k))
                        raise ValueError
            ...
            for i,new_state_dict in enumerate(new_state_dicts):
                print("Saving shard {} of {} into {}/consolidated.0{}.pth".format(i+1, num_shards, output_dir, i))
                torch.save(new_state_dict, output_dir + "/consolidated.0{}.pth".format(i))
            with open(output_dir + "/params.json", "w") as f:
                print("Saving params.json into {}/params.json".format(output_dir))
                json.dump(params, f)
...
if __name__=='__main__':
    ...
    print("Base model: {}".format(base_model_path))
    print("LoRA model(s) {}:".format(lora_model_paths))
    ...
    for lora_index, lora_model_path in enumerate(lora_model_paths):
        print("Loading LoRA {}...".format(lora_model_path))
        tokenizer = LlamaTokenizer.from_pretrained(lora_model_path)
        print("base_model vocab size: {}".format(base_model.get_input_embeddings().weight.size(0)))
        print("tokenizer vocab size: {}".format(len(tokenizer)))

        model_vocab_size = base_model.get_input_embeddings().weight.size(0)
        assert len(tokenizer) >= model_vocab_size, \
        ("The vocab size of the tokenizer {} is smaller than the vocab size of the base model {}\n"
        "This is not the intended use. Please check your model and tokenizer.".format(len(tokenizer), model_vocab_size))
        if model_vocab_size != len(tokenizer):
            base_model.resize_token_embeddings(len(tokenizer))
            print("Extended vocabulary size to {}".format(len(tokenizer)))
        ...
        if hasattr(peft.LoraModel,'merge_and_unload'):
            ...
            for k in lora_keys:
                ...
                base_model_sd[original_key] += (
                    transpose(torch.matmul(lora_model_sd[lora_b_key].float(), lora_model_sd[lora_a_key].float()), fan_in_fan_out) * lora_scaling
                )
